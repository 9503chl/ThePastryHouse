...
namespace UnityEngine.UI
{
    ...
    public class SimpleTooltip : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            contentFontData = new SimpleFontData(ContentText);
            if (UseTextFont)
            {
                contentFontData.CopyFromComponent(GetComponent<Text>());
            }
        }
        ...
        private void Update()
        {
            if (AutoPopup)
            {
                ...
                if (!Input.anyKey && EventSystemRaycaster.Instance.IsPointerOverGameObject(gameObject, SeeThrough))
                {
                    ...
                    {
                        isRollOver = true;
                        contentFontData.PasteIntoComponent(ContentText);
                        ShowTooltip(GetComponent<RectTransform>(), TooltipContent);
                    }
                }
                else if (isRollOver)
                {
        ...
        private void SetTooltip(string content)
        {
            ...
            if (ContentText != null)
            {
                ...
                ContentText.SetTextAndFitHeight(content, ContentText.fontSize);
                objectRectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, contentRectTransform.rect.height + ExpandHeight);
            }
        }
        ...
        private bool MoveTooltip(RectTransform rectTransform)
        {
            ...
            if (screenRect.Overlaps(contentRect))
            {
                ...
                Vector2 contentSize = rectTransform.rect.size + new Vector2(ExpandWidth, ExpandHeight);
                if (ContentText != null)
                {
                    contentSize += new Vector2(ContentText.fontSize, ContentText.fontSize);
                }
        ...
        public void ShowTooltip(RectTransform rectTransform, string content)
        {
        ...
        public void ShowTooltip(Vector2 screenPoint, string content)
        {
            if (isActiveAndEnabled)
            {
                if (hideRoutine != null)

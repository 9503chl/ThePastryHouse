...
{
    //[AddComponentMenu("UI/ComboBox", 36)]
    ...
    /// </remarks>
    public class ComboBox : UIBehaviour, IPointerClickHandler, ISubmitHandler, ICancelHandler
    {
        protected internal class DropdownItem : MonoBehaviour, IPointerEnterHandler, ICancelHandler
        {
            ...
            private void Update()
            {
                if (toggle && EventSystem.current.currentSelectedGameObject == gameObject)
                {
                    if ((Input.GetKeyDown(KeyCode.F4) && !(Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt))) ||
                        (Input.GetKeyDown(KeyCode.UpArrow) && (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt))) ||
                        (Input.GetKeyDown(KeyCode.DownArrow) && (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt))))
                    {
                        ComboBox dropdown = GetComponentInParent<ComboBox>();
                        if (dropdown)
                            dropdown.Hide();
                    }
                }
        ...
        public string textValue
        {
            get
            {
                if (m_Value >= 0 && m_Value < options.Count)
                    return options[m_Value].text;
                else if (m_InputField)
                    return inputField.text;
                return m_TextValue;
            }
            set { InputFieldValueChanged(value); }
        }

        public bool ignoreCase { get { return m_IgnoreCase; } set { m_IgnoreCase = value; InputFieldValueChanged(m_TextValue); } }
        ...
        void Set(int value, bool sendCallback = true)
        {
            ...
            if (Application.isPlaying && (value == oldValue || options.Count == 0))
                return;
        ...
        protected override void Awake()
        {
            ...
            if (!Application.isPlaying)
            {
                /*if (!m_InputField)
                {
                    ...
                    Text textComponent = go2.AddComponent<Text>();
                    textComponent.fontStyle = FontStyle.Normal;
                    textComponent.fontSize = 14;
                    textComponent.alignment = TextAnchor.MiddleLeft;
                    textComponent.color = new Color(0.196f, 0.196f, 0.196f, 1f);
                    textComponent.supportRichText = false;

                    m_InputField = go.AddComponent<InputField>();
                    m_InputField.placeholder = placeholder;
                    m_InputField.textComponent = textComponent;
            ...
            if (m_Value < 0 || m_Value >= options.Count)
            {
                ...
                {
                    if (string.Compare(options[i].text, m_TextValue) == 0)
                    {
                        m_Value = i;
                        break;
                    }
        ...
        public void RefreshShownValue()
        {
            ...
            if (m_CaptionText)
            {
                if (data != null && data.text != null)
                    ...
                    m_CaptionText.text = m_TextValue;
            }
        ...
        private void SetupTemplate()
        {
            ...
            Canvas popupCanvas = templateGo.EnsureComponent<Canvas>();
            ...
            if (parentCanvas != null)
            {
                ...
                for (int i = 0; i < components.Length; i++)
                {
                    ...
                    templateGo.EnsureComponent(raycasterType);
                }
            }
            else
            {
                templateGo.EnsureComponent<GraphicRaycaster>();
            }

            templateGo.EnsureComponent<CanvasGroup>();
            templateGo.SetActive(false);
        ...
        public void Show()
        {
            ...
            for (int i = 0; i < options.Count; ++i)
            {
                ...
                item.toggle.isOn = (value == i || (value == -1 && i == 0));
        ...
        protected virtual GameObject CreateBlocker(Canvas rootCanvas)
        {
            ...
            if (parentCanvas != null)
            {
                ...
                {
                    Type raycasterType = components[i].GetType();
                    blocker.EnsureComponent(raycasterType);
                }
            }
            else
            {
                // Add raycaster since it's needed to block.
                blocker.EnsureComponent<GraphicRaycaster>();
            }
        ...
        private void InputFieldValueChanged(string text)
        {
            ...
            for (int i = 0; i < options.Count; i++)
            {
                if (string.Compare(options[i].text, text, m_IgnoreCase) == 0)
                {
                    selectedIndex = i;
                    break;
                }
